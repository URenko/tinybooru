...
def update_thumb(source: str, db: sqlite3.Connection, target_size=5*2**20):
    ...
    with Image.open(origin_path) as im:
        if origin_size < target_size:
            ...
            while thumb_size >= target_size:
                ...
                im_tmp.save(thumb_file, 'JPEG', optimize=True)#'WEBP', method=6)
    ...
    r = s.post(
        ...
        data={
            ...
            'cover': b'data:' + mimetypes.guess_type(filename, strict=False)[0].encode('UTF-8') + b';base64,' + base64.b64encode(thumb_file.read()),
        },
